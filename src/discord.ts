...
export async function notifyDiscordMatchTest(
	webhookUrl: string,
): Promise<void> {
...
export function discordPayload(match: DiscordMatch) {
	const lines = [
		"@everyone",
		`[${match.match.tournamentName || "BWF"}]`,
		formatCard(match.match.team1, match.match.team2),
	];
...
function formatTeam(team?: Team): string {
	...
	const code =
		team?.countryCode ||
		team?.players?.find((player) => player.countryCode)?.countryCode;
...
function flagEmoji(countryCode?: string): string {
	...
	const code =
		COUNTRY_TO_ISO2[countryCode.toUpperCase()] || countryCode.toUpperCase();
	if (!/^[A-Z]{2}$/.test(code)) {
		return "";
	}
...
async function postDiscord(
	...
	if (!response.ok) {
		throw new Error(
			`Discord request failed: ${response.status} ${response.statusText}; body=${await response.text()}`,
		);
	}
}
